...
export const dataReact: IDataStatic[] = [
    ...
    },
    {
        key: "heading21",
        data: [
            "Context API là một tính năng được cung cấp bởi React, cho phép bạn chia sẻ dữ liệu giữa các components mà không cần phải truyền props qua nhiều lớp component. Nó tạo ra một 'context' (bối cảnh) toàn cục, nơi bạn có thể lưu trữ dữ liệu và làm cho dữ liệu đó có sẵn cho bất kỳ component nào nằm trong đó.",
            "Tại sao cần Context API?",
            "- Truyền dữ liệu qua nhiều lớp component: Khi dữ liệu cần được sử dụng ở nhiều cấp độ trong cây component, việc truyền props qua từng lớp có thể trở nên rườm rà và khó quản lý.",
            "- Tạo các theme toàn cục: Context API rất hữu ích để quản lý các theme (chủ đề) của ứng dụng, chẳng hạn như màu sắc, font chữ, v.v.",
            "- Xử lý các giá trị toàn cục: Các giá trị như ngôn ngữ, thông tin người dùng đăng nhập cũng có thể được quản lý bằng Context API.",
            "Khi nào nên sử dụng Context API?",
            "- Dữ liệu được sử dụng rộng rãi: Nếu dữ liệu cần được truy cập bởi nhiều component ở các cấp độ khác nhau, Context API là một lựa chọn tốt.",
            "- Tạo các theme hoặc cấu hình toàn cục: Context API là lý tưởng để quản lý các theme hoặc cấu hình được sử dụng trong toàn bộ ứng dụng.",
            "- Thay thế Redux trong các trường hợp đơn giản: Nếu ứng dụng của bạn không quá phức tạp và bạn không cần một thư viện quản lý state toàn cục mạnh mẽ như Redux, Context API có thể là một giải pháp nhẹ nhàng hơn.",
        ],
    },
    {
        key: "heading22",
        data: [
            "React là một thư viện JavaScript mạnh mẽ để xây dựng các giao diện người dùng, nhưng để đảm bảo ứng dụng của bạn hoạt động mượt mà và nhanh chóng, việc tối ưu hóa hiệu năng là rất quan trọng. Dưới đây là một số kỹ thuật phổ biến:",
            "1. Memoization",
            "- Ý nghĩa: Memoization là một kỹ thuật lưu trữ kết quả của một hàm đã được tính toán trước đó, để tránh tính toán lại khi các tham số đầu vào không thay đổi.",
            "- Ứng dụng trong React: React.memo là một Higher-Order Component (HOC) được sử dụng để ngăn chặn một component render lại nếu props của nó không thay đổi. Điều này đặc biệt hữu ích cho các component phức tạp hoặc đắt để render.",
            "- Cải thiện hiệu năng: Bằng cách tránh render lại các component không cần thiết, memoization giúp giảm thiểu số lần cập nhật DOM, từ đó tăng tốc độ ứng dụng.",
            "2. Virtualization",
            "- Ý nghĩa: Virtualization là kỹ thuật chỉ render các phần tử đang hiển thị trên màn hình, thay vì render toàn bộ danh sách.",
            "- Ứng dụng trong React: Các thư viện như react-window và react-virtualized cung cấp các component để thực hiện virtualization cho danh sách dài hoặc bảng dữ liệu lớn.",
            "- Cải thiện hiệu năng: Virtualization giúp giảm thiểu số lượng element được render, đặc biệt hữu ích khi làm việc với danh sách dài hoặc bảng dữ liệu lớn, giúp cải thiện khả năng cuộn và giảm thiểu việc tính toán lại layout.",
            "3. Lazy Loading",
            "- Ý nghĩa: Lazy loading là kỹ thuật trì hoãn việc tải các module hoặc component cho đến khi chúng thực sự cần thiết.",
            "- Ứng dụng trong React: React cung cấp React.lazy để lazy load các component, và Suspense để hiển thị một loading indicator trong khi component đang được tải.",
            "- Cải thiện hiệu năng: Lazy loading giúp giảm kích thước bundle ban đầu, cải thiện thời gian tải trang và giảm thiểu thời gian chờ đợi của người dùng.",
            "4. Code Splitting",
            "- Ý nghĩa: Code splitting là kỹ thuật chia nhỏ bundle JavaScript thành các chunk nhỏ hơn, giúp tải các phần của ứng dụng một cách độc lập.",
            "- Ứng dụng trong React: React cung cấp React.lazy và Suspense để thực hiện code splitting.",
            "- Cải thiện hiệu năng: Code splitting giúp giảm thiểu thời gian tải trang, đặc biệt là cho các ứng dụng lớn.",
            "Khi nào nên sử dụng các kỹ thuật này:",
            "- Memoization: Sử dụng cho các component phức tạp hoặc đắt để render, khi props không thay đổi thường xuyên.",
            "- Virtualization: Sử dụng cho danh sách dài, bảng dữ liệu lớn hoặc các thành phần có nhiều nội dung tĩnh.",
            "- Lazy Loading: Sử dụng cho các component ít khi được sử dụng hoặc các module lớn.",
            "- Code Splitting: Sử dụng cho các ứng dụng lớn, để chia nhỏ bundle và cải thiện thời gian tải trang.",
        ],
    },
    {
        key: "heading23",
        data: [
            "useState và useRef là hai hook cơ bản trong React, thường được sử dụng để quản lý trạng thái và tham chiếu trong các functional component. Mặc dù cả hai đều có vẻ tương tự, nhưng chúng phục vụ cho những mục đích khác nhau và cách sử dụng cũng khác nhau.",
            "useState",
            "- Mục đích:",
            "-- Dùng để quản lý các giá trị thay đổi theo thời gian và gây ra việc re-render component.",
            "-- Các giá trị này thường được hiển thị trực tiếp trên giao diện người dùng.",
            "- Cách hoạt động:",
            "-- Trả về một mảng gồm hai phần tử: giá trị hiện tại và một hàm để cập nhật giá trị đó.",
            "-- Khi giá trị state thay đổi, component sẽ được render lại.",
            "- Sử dụng khi:",
            "-- Bạn cần theo dõi và hiển thị các giá trị thay đổi trên giao diện.",
            "-- Bạn muốn trigger re-render khi giá trị thay đổi.",
            "useRef",
            "- Mục đích:",
            "-- Dùng để lưu trữ các giá trị không thay đổi theo thời gian và không gây ra việc re-render.",
            "-- Thường được dùng để truy cập các phần tử DOM, tạo các biến đếm, hoặc lưu trữ các giá trị cần được giữ nguyên giữa các lần render.",
            "- Cách hoạt động:",
            "-- Trả về một ref object, có một thuộc tính current để lưu trữ giá trị.",
            "-- Giá trị của ref không thay đổi khi component re-render.",
            "- Sử dụng khi:",
            "-- Bạn cần truy cập các phần tử DOM trực tiếp.",
            "-- Bạn cần lưu trữ các giá trị không liên quan đến việc render.",
            "-- Bạn muốn tạo các biến đếm không bị reset khi component re-render.",
        ],
    },
    {
        key: "heading24",
        data: [
            "useEffect và useLayoutEffect là hai hook trong React được sử dụng để thực hiện các tác vụ phụ (side effects) bên ngoài phạm vi của việc render component. Tuy nhiên, chúng có những điểm khác biệt quan trọng về thời điểm thực thi và cách chúng ảnh hưởng đến việc render giao diện người dùng.",
            "useEffect",
            "- Thời điểm thực thi: Sau khi DOM được cập nhật và trình duyệt vẽ lại giao diện.",
            "- Sử dụng cho:",
            "-- Fetch dữ liệu",
            "-- Đăng ký sự kiện",
            "-- Cleanup các tài nguyên (ví dụ: interval, timeout)",
            "-- Các tác vụ không cần phải đồng bộ hóa với DOM",
            "- Ưu điểm:",
            "-- Tránh được các vấn đề về hiệu suất khi thực hiện các tác vụ nặng nề.",
            "-- Đảm bảo giao diện người dùng được cập nhật trước khi thực hiện các tác vụ phụ.",
            "useLayoutEffect",
            "- Thời điểm thực thi: Trước khi trình duyệt vẽ lại giao diện, sau khi tất cả các DOM mutation được thực hiện.",
            "- Sử dụng cho: ",
            "-- Đo lường kích thước của các phần tử DOM",
            "-- Thực hiện các thay đổi layout",
            "-- Tạo các animation phức tạp",
            "- Ưu điểm: Đảm bảo các thay đổi DOM được thực hiện trước khi người dùng thấy chúng, tránh được các hiện tượng nhấp nháy hoặc layout shift.",
            "- Nhược điểm: Có thể gây ra các vấn đề về hiệu suất nếu sử dụng không đúng cách, vì nó chặn việc render.",
        ],
    },
];
